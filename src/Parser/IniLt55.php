...
use BugBuster\Browscap\Updater;
...
class IniLt55 extends AbstractParser
{
     ...
     */
    public function update($forceUpdate = false): void
    {
        // get updater
        $updater = Browscap::getUpdater();

        // check if an updater has been set - if not, nothing will be updated
        if (null !== $updater && false === ($updater instanceof Updater\None)) {
            ...
            if ($update) {
                ...
                if (0 === $localTimeStamp) {
                    ...
                    if (null !== $sourceVersion && $sourceVersion > $this->getVersion()) {
                        $doUpdate = true;
                    } else {
     ...
     */
    protected function checkPatternFiles(array $patternStarts): void
    {
    ...
    protected function createPatterns(): void
    {
        ...
        if (\count($matches)) {
            ...
            foreach ($matches as $match) {
                ...
                if (false !== strpbrk($match, '0123456789')) {
     ...
     * @return array
     */
    protected function getSettings($pattern, array $settings = [])
    {
        ...
        // has been found.
        if (0 === \count($settings) && \count($addSettings) > 0) {
    ...
    protected function getIniPart($pattern)
    {
        ...
        if (file_exists($file)) {
            ...
            if ($handle) {
                while (false !== ($buffer = fgets($handle))) {
                    if (0 === strpos($buffer, $patternHash)) {
     ...
     */
    protected function createIniParts(): void
    {
